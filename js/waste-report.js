...
function checkNotifications(userTasks) {
    let hasNewNotifications = false;
    userTasks.forEach(task => {
        if (task.status === 'completed' && !task.rated && !task.notified) {
            showToast(`Task #${task.id.slice(-4)} has been completed! Please rate the cleaner.`);
            task.notified = true;
