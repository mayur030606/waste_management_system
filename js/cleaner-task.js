...
export function renderCleanerDashboard(container) {
    ...
    container.innerHTML = `
        <div class="dashboard-grid">
            ...
            <!-- Completed Tasks & Ratings History -->
            <div style="grid-column: 1 / -1; margin-top: 2rem;">
                <h3 class="mb-4">Task History & Ratings</h3>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 1rem;">
                        ${completedTasks.map(task => `
                            <div class="card flex justify-between items-center" style="padding: 1rem;">
                                <div>
                                    <div class="font-bold">Task #${task.id.slice(-4)}</div>
                                    <div class="text-sm text-muted">Completed on ${new Date(task.timestamp).toLocaleDateString()}</div>
                                </div>
                                <div class="flex items-center gap-2 font-bold" style="color: var(--warning);">
                                    ${state.ratings.find(r => r.taskId === task.id) 
                                        ? state.ratings.find(r => r.taskId === task.id).rating + ' ⭐' 
                                        : '<span class="text-sm text-muted">Pending Rating</span>'}
                                </div>
                            </div>
                        `).join('')}
                    </div>
